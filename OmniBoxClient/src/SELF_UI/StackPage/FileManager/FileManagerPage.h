...
private slots:
    ...
    // ================= 右键空白区域动作 =================
    void onActionUploadFile();                                                                              // 点击“上传文件”
...
private:
    ...
    // 辅助 UI 渲染
    void InsertRowToUI(int64_t nodeId, const QString& name, bool isDir, 
        int64_t size, const QString& updateTime);
    QString FormatSize(int64_t bytes);                                                                      // 格式化文件大小 (KB, MB, GB)

    // 文件传输
    void StartUploadFile(const QString& localPath, int64_t targetFolderId);                                 // 上传文件
...
private:
    ...
    // 右键菜单
    QMenu* context_menu_;                                                                                   // 文件/文件夹被选中时的右键菜单
    QMenu* empty_context_menu_;                                                                             // 空白处的右键菜单
