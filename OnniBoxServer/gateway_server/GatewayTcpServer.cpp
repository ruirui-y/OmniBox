...
void GatewayTcpServer::HandleHttpRequest(const std::shared_ptr<TcpConnection>& conn, Buffer* buffer)
{
    ...
    // 👑 架构师补丁：剥离 GET 请求的参数 (把 /check_file?hash=123 变成 /check_file)
    // base_uri = /check_file
    ...
    // 构造路由键，例如 "POST /upload_chunk" 或 "GET /check_file"
    std::string route_key = std::string(method) + " " + base_uri;

    // 1. 去路由表里查，有没有精确匹配的 API 接口？
    auto it = router_.find(route_key);
    if (it != router_.end())
    {
        // 找到了！直接执行对应的业务函数 (注意传进去的是 full_uri，因为业务层可能需要提取参数)
    ...
    {
        LOG_ERROR << "❌ 404 拦截: 找不到路由 -> " << route_key;
        conn->Send("HTTP/1.1 404 Not Found\r\n\r\n 404: 路由未配置");
        buffer->RetrieveAllAsString(); // 清空错误包
        conn->ForceClose();
    }
}
...
*/
// ==============================================================================
void GatewayTcpServer::HandleStaticResource(const std::shared_ptr<TcpConnection>& conn, Buffer* buffer, const std::string& method, const std::string& uri, const std::string& headers, size_t header_end_pos)
{
...
*/
// ==============================================================================
void GatewayTcpServer::HandleUploadChunk(const std::shared_ptr<TcpConnection>& conn, Buffer* buffer, const std::string& method, const std::string& uri, const std::string& headers, size_t header_end_pos)
{
    // 1. 解析Content-Length
    size_t cl_pos = headers.find("Content-Length: ");
...
*/
// ==============================================================================
void GatewayTcpServer::HandleCheckFile(const std::shared_ptr<TcpConnection>& conn, Buffer* buffer, const std::string& method, const std::string& uri, const std::string& headers, size_t header_end_pos)
{
